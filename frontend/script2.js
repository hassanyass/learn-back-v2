...
(function () {
  ...
  var dom = {
    ...
    // Slide Deck Chat Mirror (removed — now using simple iframe)

    // Slides / action cards (legacy right-panel view — kept for Back button)
    ...
    // Concept Card (left panel — single current thought)
    ...
    // KWL View (right panel — full thought stream history)
    ...
    // Header Dropdown (End Session) & Mute
    ...
    // New Refactor Elements (slide deck now uses a simple iframe, no resizer)
  };

  // ─── State ────────────────────────────────────────────────
  // Read topics from localStorage if available
  var savedCategories = [];
  try {
    var loaded = localStorage.getItem('learnback_categories');
    if (loaded) {
      var parsed = JSON.parse(loaded);
      savedCategories = parsed.map(function (c) { return typeof c === 'string' ? c : (c.title || c.name || "Untitled Segment"); });
    }
  } catch (e) {
  ...
  var state = {
    currentTopic: 0,
    topics: savedCategories.length > 0 ? savedCategories : defaultTopics,
  ...
  function refreshTopicUI() {
    var isComplete = state.currentTopic >= state.topics.length;
    var t = isComplete ? 'Session Complete' : state.topics[state.currentTopic];
    ...
    if (dashCount) {
      if (isComplete) dashCount.textContent = 'ALL TOPICS COMPLETED';
  ...
  function confirmSkip() {
    ...
    if (sessionId) {
      fetch('http://127.0.0.1:8000/api/session/skip', {
        method: 'POST',
  ...
  if (btnStartSession) {
    btnStartSession.addEventListener('click', function () {
      ...
      fetch('http://127.0.0.1:8000/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ session_id: sessionId, message: "init", action: "INITIALIZE" })
      })
        .then(function (response) { return response.json(); })
        .then(function (data) {
          typeMessage(bubbleToType, data.kido_response).then(function () {
            setChatLockout(false);
            updateHud('waiting');
          });
        })
        .catch(function (e) {
          typeMessage(bubbleToType, "I'm feeling a little glitchy. Can we try again?").then(function () {
            setChatLockout(false);
            updateHud('waiting');
          });
        });
  ...
  function sendMessage(sourceText) {
    ...
    fetch('http://localhost:8000/api/chat', {
      ...
      .then(function (data) {
        var aiText = data.kido_response || "I'm thinking really hard... can you say that again?";
        var isGeneric = (data.eval_status === 'conversational' || data.eval_status === 'off_topic');

        if (data.eval_status === 'plagiarized') {
          updateHud('needs_detail'); // Alert the user via HUD without saving text
        ...
        var newProgress = typeof data.progress_bar_percentage === 'number' ? data.progress_bar_percentage : state.progress;
        var delta = newProgress - state.progress;

        // Derive HUD state from progress movement
        var hudStateKey, type;
        if (delta > 0) {
          hudStateKey = 'correct';
          type = 'mastered';
        } else if (delta < 0) {
          ...
          hudStateKey = 'needs_detail';
          type = 'developing';
        }

        // Update misconceptions from backend
        if (data.misconceptions_list && Array.isArray(data.misconceptions_list)) {
          data.misconceptions_list.forEach(function (m) {
            if (!state.misconceptions.find(function (x) { return x.text === m; })) {
              state.misconceptions.unshift({ text: m, time: ts() });
            }
          });
          if (data.misconceptions_list.length > 0) {
            renderMisconceptionsList();
            showMisconceptionsBadge();
          }
        }

        // Update kido notes to knowledge stream (mastered entries from backend)
        if (data.kido_notes && Array.isArray(data.kido_notes)) {
          data.kido_notes.forEach(function (note) {
            if (!state.knowledge.find(function (k) { return k.text === note; })) {
              addKnowledge(note, 'mastered', 0);
            }
          });
        }

        // Set progress from real BKT score
        setProgress(newProgress);
        updateHud(hudStateKey);
        addKnowledge(aiText, type, delta);

        // Save progress to session store
        if (window.SessionStore) {
          var s = window.SessionStore.getSession();
          window.SessionStore.saveSession(s.sessionId, s.topicIndex, newProgress);
        }

        // Type Kido's real AI-generated response
        typeMessage(bubbleToType, aiText).then(function () {
          setChatLockout(false);
        });
      })
  ...
  if (btnSparkEl) {
    btnSparkEl.addEventListener('click', function () {
      ...
      fetch('http://127.0.0.1:8000/api/hint', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        .then(function (data) {
          var text = data.hint_text || "Try thinking about the first principles behind this topic!";
          typeMessage(bubbleToType, text).then(function () {
            setChatLockout(false);
          });
        })
        .catch(function (err) {
          typeMessage(bubbleToType, "I'm meant to give a hint, but my circuits are fuzzy right now.").then(function () {
            setChatLockout(false);
          });
        });
    });
  ...
  // ─── EXCALIDRAW MCP DUMMY INTEGRATION ────────────────────────
  function injectExcalidrawApp() {
    var text = "Can you draw a concept map of what we learned?";
    var currentId = state.messageCount++;
    addMessage(text, 'user', currentId);

    // Set card to thinking state immediately
    updateConceptCard({ text: 'I\'ll generate a concept map visualizing our progress.', type: 'thinking', delta: 0 });

    var delay = 1200 + Math.random() * 800; // slightly longer delay for "app generation"
    setTimeout(function () {
      var aiText = "Here is an interactive diagram showing the core concepts of Neural Networks we discussed.";
      var type = 'mastered';
      var delta = calculateDelta(type);

      bumpProgress(delta);

      // Create standard text message introduction
      addMessage(aiText, 'ai');
      addKnowledge(aiText, type, delta);

      // Create specially formatted App Message containing the Excalidraw iframe
      setTimeout(function () {
        var appMsgWrapper = document.createElement('div');
        appMsgWrapper.className = 'message message--ai';

        var senderLabel = document.createElement('span');
        senderLabel.className = 'message__sender';
        senderLabel.textContent = 'KIDO (Excalidraw MCP)';

        var appBubble = document.createElement('div');
        appBubble.className = 'message__bubble message__bubble--app';

        var iframe = document.createElement('iframe');
        iframe.src = "excalidraw-mcp-mock.html";
        iframe.title = "Excalidraw diagram";
        iframe.allowFullscreen = true;

        // Listen for the custom "excalidraw-ready" event from the mock before drawing
        const handleIframeReady = (e) => {
          if (e.data && e.data.type === "excalidraw-ready") {
            window.removeEventListener("message", handleIframeReady);

            // Wait an extra small moment for the React DOM to paint
            setTimeout(function () {

              let els = [];

              // Step 1: Draw Hub Box (Fuzzy Logic) - Plum (#925e78)
              els.push({ type: "rectangle", x: 300, y: 300, width: 180, height: 60, strokeColor: "#925e78", backgroundColor: "transparent", fillStyle: "solid", strokeWidth: 2, roughness: 1, id: "hub" });
              els.push({ type: "text", x: 325, y: 318, fontSize: 20, fontFamily: 1, text: "Fuzzy Logic", strokeColor: "#925e78" });

              iframe.contentWindow.postMessage({ type: "excalidraw", event: "import", data: { elements: JSON.parse(JSON.stringify(els)) } }, "*");

              // Step 2: Draw TR Node (Degrees of Truth) - Sage Green (#5da271)
              setTimeout(function () {
                els.push({ type: "rectangle", x: 550, y: 150, width: 220, height: 60, strokeColor: "#5da271", backgroundColor: "transparent", fillStyle: "solid", strokeWidth: 2, roughness: 1, id: "tr-node" });
                els.push({ type: "text", x: 570, y: 168, fontSize: 20, fontFamily: 1, text: "Degrees of Truth", strokeColor: "#1a1f36" });
                els.push({ type: "arrow", x: 440, y: 300, width: 110, height: -120, strokeColor: "#5da271", strokeWidth: 2, roughness: 1, points: [[0, 0], [110, -120]], roundness: { type: 2 } });
                els.push({ type: "text", x: 450, y: 220, fontSize: 16, fontFamily: 1, text: "replaces True/False with", strokeColor: "#5da271" });

                iframe.contentWindow.postMessage({ type: "excalidraw", event: "import", data: { elements: JSON.parse(JSON.stringify(els)) } }, "*");

                // Step 3: Draw BR Node (Membership Functions) - Sage Green
                setTimeout(function () {
                  els.push({ type: "rectangle", x: 550, y: 450, width: 260, height: 60, strokeColor: "#5da271", backgroundColor: "transparent", fillStyle: "solid", strokeWidth: 2, roughness: 1, id: "br-node" });
                  els.push({ type: "text", x: 565, y: 468, fontSize: 20, fontFamily: 1, text: "Membership Functions", strokeColor: "#1a1f36" });
                  els.push({ type: "arrow", x: 440, y: 360, width: 110, height: 90, strokeColor: "#5da271", strokeWidth: 2, roughness: 1, points: [[0, 0], [110, 90]], roundness: { type: 2 } });
                  els.push({ type: "text", x: 510, y: 390, fontSize: 16, fontFamily: 1, text: "calculated using", strokeColor: "#5da271" });

                  iframe.contentWindow.postMessage({ type: "excalidraw", event: "import", data: { elements: JSON.parse(JSON.stringify(els)) } }, "*");

                  // Step 4: Draw BL Node (Defuzzification - Unverified) - Yellow Dashed
                  setTimeout(function () {
                    els.push({ type: "rectangle", x: 0, y: 450, width: 200, height: 60, strokeColor: "#f6ae2d", strokeStyle: "dashed", backgroundColor: "transparent", fillStyle: "solid", strokeWidth: 2, roughness: 1, id: "bl-node" });
                    els.push({ type: "text", x: 25, y: 468, fontSize: 20, fontFamily: 1, text: "Defuzzification", strokeColor: "#1a1f36" });
                    els.push({ type: "arrow", x: 300, y: 360, width: -140, height: 90, strokeColor: "#f6ae2d", strokeWidth: 2, roughness: 1, strokeStyle: "dashed", points: [[0, 0], [-140, 90]], roundness: { type: 2 } });
                    els.push({ type: "text", x: 40, y: 390, fontSize: 16, fontFamily: 1, text: "turns it back to crisp numbers?", strokeColor: "#f6ae2d" });

                    iframe.contentWindow.postMessage({ type: "excalidraw", event: "import", data: { elements: JSON.parse(JSON.stringify(els)) } }, "*");

                    // Step 5: Draw Margin Note
                    setTimeout(function () {
                      els.push({ type: "text", x: 450, y: 560, fontSize: 20, fontFamily: 1, text: "It's not just 0 or 1, it's the gray area in between! ✍️", strokeColor: "#1a1f36" });
                      iframe.contentWindow.postMessage({ type: "excalidraw", event: "import", data: { elements: JSON.parse(JSON.stringify(els)) } }, "*");
                    }, 800);
  ...
  // 4-node mock dataset
  var KC_NODES = [
    { id: 1, title: 'Weights', thought: 'Weights are just numbers that stay the same.', status: 'pending', correction: '' },
    { id: 2, title: 'Activation', thought: 'It decides if the neuron fires based on the sum of inputs.', status: 'pending', correction: '' },
    { id: 3, title: 'Backprop', thought: 'Backprop sends error signals backward through layers to adjust weights.', status: 'pending', correction: '' },
    { id: 4, title: 'Loss Function', thought: 'It measures how far my output is from the right answer.', status: 'pending', correction: '' },
  ];
  // Connection pairs [fromId, toId]
  var KC_CONNECTIONS = [[1, 2], [2, 3], [3, 4]];
  ...
  if (dom.btnRequestGraph) {
    dom.btnRequestGraph.addEventListener('click', injectKnowledgeCubeCard);
  }
  ...
  if (dom.btnModalGrade) {
    dom.btnModalGrade.addEventListener('click', function () {
      alert('Finalize & Grade triggered! Redirecting to Session Summary...');
      if (dom.modalFinalize) dom.modalFinalize.setAttribute('hidden', '');
  ...
  // Seed dummy thought entries for UI preview
  var dummySeed = [
  ...
  for (var si = dummySeed.length - 1; si >= 0; si--) {
    state.knowledge.unshift(dummySeed[si]);
  }
  ...
  // Seed dummy chat messages for UI preview
  var dummyChat = [
  ...
  dummyChat.forEach(function (m) { addMessage(m.text, m.sender); });
  ...
  var MindMapManager = {
    getData: function () {
      return {
        nodes: [
          { id: 'n1', label: 'Neural Networks', status: 'verified', def: 'A system of interconnected nodes inspired by the human brain.' },
          { id: 'n2', label: 'Layer', status: 'verified', def: 'A collection of neurons operating at a specific depth in the network.' },
          { id: 'n3', label: 'Weights', status: 'unverified', def: 'The strength of connection between two neurons.' },
          { id: 'n4', label: 'Backpropagation', status: 'fallback', def: 'Randomly adjusting weights until accuracy improves (INCORRECT).' }
        ],
        links: [
          { source: 'n1', target: 'n2', type: 'confirmed' },
          { source: 'n2', target: 'n3', type: 'confirmed' },
          { source: 'n1', target: 'n4', type: 'fallback' }
        ]
      };
    },
  ...
  function openSlideDeck() {
    ...
    if (chatPanel) chatPanel.classList.add('slides-open');
    if (chatPanel && !chatPanel.style.getPropertyValue('--chat-split-width')) {
      chatPanel.style.setProperty('--chat-split-width', '50%');
    }
    if (pdfPane) {
  ...
  function closeSlideDeck() {
    ...
    if (chatPanel) {
      ...
      chatPanel.style.removeProperty('--chat-split-width'); // Clean up any drag inline styles
    }
    if (pdfPane) {
  ...
  if (dragHandle && chatContainer && chatPanel) {
    function applySplitFromClientX(clientX) {
      ...
      var minWidth = rect.width * 0.20;
      var maxWidth = rect.width * 0.80;
      var newWidth = Math.min(maxWidth, Math.max(minWidth, clientX - rect.left));
      var widthPercent = (newWidth / rect.width) * 100;

      chatPanel.style.setProperty('--chat-split-width', widthPercent + '%');
      chatContainer.style.flex = '';
      chatContainer.style.width = '';

      var slideDeck = document.getElementById('slide-deck-view');
      if (slideDeck) {
        slideDeck.style.flex = '';
        slideDeck.style.width = '';
      }
    }
    ...
    dragHandle.addEventListener('pointermove', function (e) {
      if (!isDragging || e.pointerId !== resizePointerId) return;
...
document.addEventListener('DOMContentLoaded', async () => {
  if (window.SessionStore) {
    ...
    if (sessionData) {
      if (typeof setProgress === 'function') {
        setProgress(sessionData.progress_bar_percentage);
      }
      console.log('Session Resumed:', sessionData);
    } else {
      const newSessionId = 'sess_' + Math.random().toString(36).substring(2, 9);
      window.SessionStore.saveSession(newSessionId, 0, 0);
    }
  }

  const endBtn = document.getElementById('btn-header-actions');
  if (endBtn) {
    endBtn.addEventListener('click', () => {
      if (window.SessionStore) window.SessionStore.endSession();
    });
  }
  ...
  const finishModalBtn = document.getElementById('btn-modal-grade');
  if (finishModalBtn) {
    finishModalBtn.addEventListener('click', () => {
      if (window.SessionStore) window.SessionStore.endSession();
      // Navigate to the premium feedback summary page
      window.location.href = 'feedback.html';
    });
  }
});
